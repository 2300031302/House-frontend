...
import { Users, Wrench, Calendar, DollarSign, TrendingUp, Eye, CheckCircle, XCircle, AlertTriangle } from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  const servicePopularity = state.services.map(service => ({
    name: service.name,
    bookings: state.bookings.filter(b => b.serviceId === service.id).length,
  ...
  const tabs = [
    ...
    { id: 'professionals', name: 'Professionals', icon: Wrench },
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-white to-indigo-50 pt-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">Manage your HomeServices platform</p>
        </div>
          ...
          <div className="bg-white/60 backdrop-blur-sm border border-white/20 rounded-2xl p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-green-600">{totalProfessionals}</p>
              </div>
        ...
        {/* Tabs */}
        <div className="mb-8">
          <div className="flex space-x-1 bg-white/60 backdrop-blur-sm border border-white/20 rounded-xl p-1">
            {tabs.map((tab) => {
              const Icon = tab.icon;
              return (
                <button
        ...
        {activeTab === 'overview' && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            {/* Service Popularity */}
            <div className="bg-white/60 backdrop-blur-sm border border-white/20 rounded-2xl p-6 lg:col-span-2">
                  ...
                  {state.users.map((user) => {
                    ...
                    return (
                      <tr key={user.id} className="hover:bg-gray-50/50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{userBookings.length}</td>
        ...
        {activeTab === 'professionals' && (
          <div className="bg-white/60 backdrop-blur-sm border border-white/20 rounded-2xl overflow-hidden">
            <div className="p-6 border-b border-gray-200/50">
              <h3 className="text-xl font-semibold text-gray-900">Professional Management</h3>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-gray-50/50">
                ...
                <tbody className="divide-y divide-gray-200/50">
                  {state.professionals.map((professional) => {
                    const professionalBookings = state.bookings.filter(b => b.professionalId === professional.id);
                    const earnings = professionalBookings
                      .filter(b => b.status === 'completed')
                      .reduce((sum, booking) => sum + booking.price, 0);
                    
                    return (
                      <tr key={professional.id} className="hover:bg-gray-50/50">
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center">
                            <img className="h-10 w-10 rounded-full object-cover" src={professional.image} alt={professional.name} />
                            <div className="ml-4">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{professional.specialty}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                          <div className="flex items-center">
                            <span className="text-yellow-400">★</span>
                            <span className="ml-1">{professional.rating} ({professional.reviewCount})</span>
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{professionalBookings.length}</td>
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-gray-200/50">
                  {state.bookings.map((booking) => {
                    ...
                    const professional = state.professionals.find(p => p.id === booking.professionalId);
